...
#define CIPHER_LEN (1+2*g_uNumbits/8 + plaintextLen_in+ HASH_NUMBITS/8)
	...
	printf("\n ****g_uNumbits = %d CIPHER_LEN=%d\n",g_uNumbits,CIPHER_LEN);
	/* 检查明文是否有效 */
	...
	printf("*****ECC_Encrypt() pPlaintext_in=%s plaintextLen_in=%d !\n",pPlaintext_in,plaintextLen_in);

	if (!(ECC_Encrypt(pCipher_out, group, G, P, pPlaintext_in, plaintextLen_in)))
	{
		printf("*****ECC_Encrypt() sucess!\n");
		/* 密文长度 */
